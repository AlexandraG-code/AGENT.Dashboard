 ...
 *
 * Единица как нижняя граница здесь была ошибкой: при расходе в центы график
 * делился на выдуманный доллар, все полосы ложились в минимум, а подписью пика
 * показывался тот самый доллар. Пустой набор отдаёт ноль, а деление на него
 * закрывает вызывающий код.
 */
export function peakOf<T>(items: T[], value: (item: T) => number): number {
	return Math.max(...items.map(value), 0)
}
